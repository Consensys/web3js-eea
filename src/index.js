...
function EEAClient(web3, chainId) {
    ...
    web3.eea = {
        sendRawTransaction: (options) => {
            ...
            return web3.eth.getTransactionCount(ethUtil.privateToAddress(privateKeyBuffer).toString('hex'))
                .then(nonce => {
                    tx.nonce = nonce;
                    ...
                    return axios.post(host,
                        {"jsonrpc": "2.0", "method": "eea_sendRawTransaction", "params": [signedRlpEncoded], "id": 1}
                    )
                });

        },
        getTransactionReceipt: (txHash, enclavePublicKey, delay = 100, retries = 30) => {
